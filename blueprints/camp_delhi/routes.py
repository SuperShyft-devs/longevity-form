# routes.py - Camp Delhi Blueprint Routes (Simpler, no time slots)
...
def send_to_api(booking_data):
    print(f"[CAMP_DELHI] Sending to API: {booking_data}")
    try:
        ...
        if response.ok:
            print(f"[CAMP_DELHI] API Success: {response.json()}")
            return True
        else:
            print(f"[CAMP_DELHI] API Error: {response.status_code} - {response.text}")
            return False
    except Exception as e:
        print(f"[CAMP_DELHI] API Exception: {str(e)}")
        return False
...
def send_email_async(booking_data):
    try:
        ...
        if email_success:
            print("[CAMP_DELHI] Booking notification email sent successfully (async)")
        else:
            print("[CAMP_DELHI] Failed to send booking notification email (async)")
    except Exception as e:
        print(f"[CAMP_DELHI] Exception in async email sending: {str(e)}")


# Main Routes
@camp_delhi_bp.route("/")
def index():
    """Main registration form for Delhi camp"""
    return render_template("camp_delhi/index.html")


@camp_delhi_bp.route("/api/config/minimum_days_ahead")
...
@camp_delhi_bp.route("/booking_success/<int:booking_id>")
def booking_success(booking_id):
    """Display booking confirmation"""
    booking = get_booking_by_id(booking_id)
    if not booking:
        return redirect(url_for('camp_delhi.index'))
    return render_template("camp_delhi/success.html", booking=booking)


@camp_delhi_bp.route("/submit_booking", methods=["POST"])
def submit_booking():
    try:
        is_valid, error_message, booking_data = validate_booking_data(request.form)
        if not is_valid:
            return render_template("camp_delhi/error.html", message=error_message)
        ...
        if api_enabled:
            try:
                if send_to_api(booking_data):
                    print(f"[CAMP_DELHI] Booking #{booking_id}: API integration successful")
                else:
                    print(f"[CAMP_DELHI] Booking #{booking_id}: API integration failed (booking still saved)")
            except Exception as api_error:
                print(f"[CAMP_DELHI] Booking #{booking_id}: API integration error - {str(api_error)}")

        # Send email notification asynchronously
        threading.Thread(target=send_email_async, args=(booking_data,)).start()

        return redirect(url_for("camp_delhi.booking_success", booking_id=booking_id))
    except Exception as e:
        return render_template("camp_delhi/error.html", message=f"Unexpected error: {str(e)}")


# Admin Routes
@camp_delhi_bp.route("/admin")
@admin_required
def admin():
    """Admin dashboard for Delhi camp"""
    bookings = get_all_bookings()
    return render_template("camp_delhi/admin.html", bookings=bookings)


@camp_delhi_bp.route("/admin/delete_booking/<int:booking_id>", methods=["POST"])
@admin_required
def delete_booking(booking_id):
    ...
    try:
        ...
        if success:
            print(f"[CAMP_DELHI] Booking #{booking_id} for {booking_name} deleted")
        else:
            print(f"[CAMP_DELHI] Booking not found")
        return redirect(url_for("camp_delhi.admin"))
    except Exception as e:
        print(f"[CAMP_DELHI] Error deleting booking: {str(e)}")
        return redirect(url_for("camp_delhi.admin"))


@camp_delhi_bp.route("/admin/delete_records")
@admin_required
def delete_records():
    """Delete records page"""
    bookings = get_all_bookings()
    return render_template("camp_delhi/delete_records.html", bookings=bookings)


@camp_delhi_bp.route("/admin/config", methods=["GET", "POST"])
@admin_required
def admin_config():
    ...
    if request.method == "POST":
        try:
            ...
            if not is_valid:
                return render_template("camp_delhi/admin_config_new.html", config=config_manager.get_all(), errors=errors)

            config_manager.update_multiple(config_data)
            return render_template("camp_delhi/admin_config_new.html", config=config_manager.get_all(), success="Configuration updated!")
        except Exception as e:
            return render_template("camp_delhi/admin_config_new.html", config=config_manager.get_all(), errors=[f"Error: {str(e)}"])

    return render_template("camp_delhi/admin_config_new.html", config=config_manager.get_all())


@camp_delhi_bp.route("/admin/config/email-management")
@admin_required
def admin_email_management():
    """Email recipients management page"""
    return render_template("camp_delhi/admin_email_management.html")


@camp_delhi_bp.route("/admin/config/reset", methods=["POST"])
@admin_required
def reset_config():
    ...
    try:
        ...
        return render_template("camp_delhi/admin_config.html", config=config_manager.get_all(), errors=[f"Error: {str(e)}"])


@camp_delhi_bp.route("/admin/config/test-email", methods=["POST"])
@admin_required
def test_email():
...
@camp_delhi_bp.route("/admin/config/recipient-emails", methods=["GET", "POST"])
@admin_required
