...
def landing():
    ...
    from blueprints.camp.database import get_booking_count as get_camp_count
    
    try:
        bio_ai_count = get_bio_ai_count()
        bio_ai_free_count = get_bio_ai_free_count()
        camp_count = get_camp_count()
        total_count = bio_ai_count + bio_ai_free_count + camp_count
    except:
        bio_ai_count = bio_ai_free_count = camp_count = total_count = 0
...
def admin_dashboard():
    ...
    from blueprints.camp.database import get_all_bookings as get_camp_bookings
    ...
    try:
        ...
        if form_filter in ['all', 'camp']:
            camp_bookings = [(b, 'camp') for b in get_camp_bookings()]
    except Exception as e:
        print(f"Error fetching bookings: {e}")
    
    # Combine all bookings
    all_bookings = bio_ai_bookings + bio_ai_free_bookings + camp_bookings
...
def admin_delete_booking(form_name, booking_id):
    ...
    try:
        if form_name == 'bio_ai':
            ...
            from blueprints.camp.database import delete_booking_by_id
            delete_booking_by_id(booking_id)
